...
export default function DocumentCard({id, document}: DocumentProps) {
    ...
    useEffect(() => {
        if (user) {
            const isLiked = document.likes.find((like) => like.userId === user.id);
            setLiked(!!isLiked);
        }
    }, [user, document]);
    ...
    const handleLike = useCallback(async () => {
        if (user?.id) {
            const userLike = document.likes.find((like) => like.userId === user.id);
        ...
        <>
            <Dialog>
                <DialogTrigger asChild>
                    <Card
                        className="hover:-translate-y-0 sm:hover:-translate-y-1 bg-neutral-white  cursor-pointer block transition-transform rounded-lg border border-foreground/5 will-change-transform hover:shadow-lg">
                        <CardHeader>
                        ...
                        <CardContent className="-mt-2 space-y-3">
                            ...
                            <div className="flex gap-4 mt-3 ">
                                ...
                                <Button
                                    size={
                                        document.likes.length > 0 ? "default" : "icon"
                                    }
                                    variant={"ghost"}
                                ...
                                >
                                    ...
                                    />
                                    {document.likes.length > 0 && document.likes.length}
                                </Button>
                            </div>
                        </CardContent>
                    </Card>
                ...
                <DialogContent className={"max-w-4xl"}>
                    <div className={"grid grid-cols-2 gap-3"}>
                        <div className="">
                            <embed src={document.fileURL} width="100%" className={"rounded-md"} height="600px"></embed>
                        </div>
                        <div className="p-3">
                            <div className="flex flex-col gap-3">
                                ...
                                <div className="flex flex-col gap-3">
                                    ...
                                    <p className={"text-foreground/50"}>Publié le {document.createdAt?.toLocaleDateString("fr-FR", {
                                        year: "numeric",
                                        month: "long",
                                        day: "numeric"
                                    })}</p>
                            ...
                            <div className="mt-3">
                                <p className="text-base text-justify tracking-tight">
                                    {document.description}
                                </p>
